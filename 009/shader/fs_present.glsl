...
vec3 get_ssr(vec3 N, vec3 wPos, float base_depth) {
	...
	for(int i = 1; i < 16; i++) {
		...
		float diff = base_depth - base_depth;
		if(auv.z < kdepth) {
			const float thick = 0.01;
			const float diff = kdepth - auv.z;
			if( diff > thick ) {
				dens += 0.5;
			}
			col += mix(col, GetColor(auv.xy), 0.5) * 0.3;
		}
		dens += 1.0;
	}
	col /= dens;
...
void main() {
	...
	FragColor.xyz  += get_ssr(N, world_pos, depth);
	FragColor.w     = 1.0;
	return;
}
