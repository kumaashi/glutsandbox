...
vec3 get_dist_pos(int id) {
	...
	float x =  cos(ipos.y * idmult * 5.11112 * 0.01 + info.w * 0.031) * 4.0; // ((noise(rpos.xyz * width + vec3(idmult))) * 2.0 - 1.0);
	float y =  sin(ipos.x * idmult * 7.11112 * 0.02 + info.w * 0.032) * 3.0; // ((noise(rpos.yzx * width + vec3(idmult))) * 2.0 - 1.0);
	float z = -cos(ipos.z * idmult * 9.11112 * 0.01 + info.w * 0.033) * 3.0; // ((noise(rpos.zxy * width + vec3(idmult))) * 2.0 - 1.0);
	return vec3(x, y, z);
}
