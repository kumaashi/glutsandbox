glutsandbox
=============

- 001 - basic.

![image](https://github.com/kumaashi/glutsandbox/blob/master/image/image000.png)

- 002 - Instancing.

![image](https://github.com/kumaashi/glutsandbox/blob/master/image/image002.png)

- 003 - Shadow Map.

![image](https://github.com/kumaashi/glutsandbox/blob/master/image/image003.png)

- 004 - Geometry Shader and generate normal.

![image](https://github.com/kumaashi/glutsandbox/blob/master/image/image004.png)


- 005 - Screen Space Ambient Occlusion(SSAO). Not yet apply parameter adjustment.

![image](https://github.com/kumaashi/glutsandbox/blob/master/image/image005.png)

- 006 - cheap deferred shading, and blur etc.

![image](https://github.com/kumaashi/glutsandbox/blob/master/image/image006.png)

