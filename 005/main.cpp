...
struct Screen {
	...
	Screen(int w, int h) {
		...
		std::vector<Screen::Info> vrendertargetnumber = {
			{SHADOW_MAP_WIDTH, SHADOW_MAP_HEIGHT, 0}, //shadow pass
			{w, h, 4}, //rendering
			{w, h, 1}, //post
		};
...
void renderFrameFunc(void) {
	...
	
	{
		...
		for(auto &t : vdepthtex) {
			t->Bind(0, glGetUniformLocation(shader->Get(), name.c_str()), is_gen_mipmap, true);
		}
	...
	
	{
		...
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		{
			...
			for(auto &t : vcolortex) {
				...
				t->Bind(index, glGetUniformLocation(shader->Get(), name.c_str()), is_gen_mipmap);
				index++;
			}
			for(auto &t : vdepthtex) {
				auto name = std::string("depth_tex");
